...
import ChangeImg from "/public/file/changeImg.svg";
import DeleteImg from "/public/file/deleteImg.svg";
...
`;
const ProfileImage: React.FC<ImageStyle> = ({
  mode = "noChange",
  ...props
}) => {
  const { data: session, status } = useSession();
  const [displayImage, setDisplayImage] = useState<string | null | undefined>(
    null
  );
  ...
  return (
    <ImgContainer width={props.size[0]} height={props.size[1]} mode={mode}>
      <ProfileImg
        src={
