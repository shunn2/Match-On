...
const PostContainer = styled.div<{ type: string | string[]; isMe: string }>`
  width: ${(props) =>
    props.type === "team" && props.isMe === "1" ? "65%" : "100%"};
  height: 100%;
  display: flex;
  /* border: ${(props) =>
    props.type === "team" && props.isMe === "1" ? "1px solid black" : ""}; */
  ...
  return (
    <Container>
              ...
              <Info>
                ...
                <div className="created">
                  {typeof detail.createdAt === "string" &&
                    format(parseISO(detail.createdAt), "yyyy-MM-dd")}
                </div>
              </Info>
            </div>
            {detail.isMe === "1" && (
      ...
      {type === "team" && detail.isMe === "1" && (
        <ResumeList
          resumeList={detail.resumes}
          type={lectureName}
          index={Number(lectureIdx)}
        />
      )}
      {isResumeModalOpen && (
        <ResumeModal
          isOpen={isResumeModalOpen}
          handleOpen={handleResume}
          postIdx={postIdx}
          type={"lecture"}
        />
      )}
